...
'''
Version 2 - instead of training to 15 epochs, trains until the change in cost function < e.g. 0.001
          - also removed saving of weights and images after each epoch and added saving of final weights
'''
...
class dA(object):
    ...
    def __init__(self, numpy_rng, theano_rng = None,
                 input = None, n_visible = 12, n_hidden = 12,
        ...
        if not W:
            ...
            initial_W = numpy.asarray(
                numpy_rng.uniform(low = -4 * numpy.sqrt(6. / (n_hidden + n_visible)),
        ...
        if not bhid:
            bhid = theano.shared(value = numpy.zeros(n_hidden, dtype = theano.config.floatX),
                                 name = 'b',
                                 borrow = True)
...
def test_dA(dataset, numFeatures, numHidden,
            minCorruptionLevel = 0.0, maxCorruptionLevel = 1.0, corruptionLevelStep = 0.1,
            learning_rate = 0.1, 
            batch_size = 20, output_folder = 'dA_plots', 
            deltaCostStopThreshold = 0.001, learningRateBoostFactor = 1.5):
    '''
    Inputs:
    ...
    for corruptionLevel in numpy.arange(minCorruptionLevel, 
                                        maxCorruptionLevel + corruptionLevelStep, 
                                        corruptionLevelStep):
        ...
        da = dA(numpy_rng = rng, theano_rng = theano_rng,
                input = x, n_visible = numFeatures, n_hidden = numHidden)
        ...
        numpy.savetxt('weights_' + outputFnRoot + '.csv', weights, delimiter = ',')
        ...
        record = {'Corruption': corruptionLevel,
                  '# Feature': numFeatures,
        ...
        print >> sys.stderr, ('The ' + strCorruptionLevel + '% corruption code for file ' +
