...
def test_rbm(learning_rate = 0.1, training_epochs = 15,
    ...
    rbm = RBM(input = x, n_visible = n_visible, n_hidden = n_hidden, 
    ...
    for epoch in xrange(training_epochs):
        ...
        weights = rbm.W.get_value(borrow = True).T
        image = Image.fromarray(tile_raster_images(X = weights, img_shape = (1, n_visible),
                                                   tile_shape=(n_hidden, 1), tile_spacing=(5, 5)))
        image.save('filters_' + '_epoch_' + str(epoch) + '_nin_' + str(n_visible) + '_nhdn_' + str(n_hidden) + '.png')
        plotting_stop = time.clock()
        plotting_time += (plotting_stop - plotting_start)
        # Save weights
        numpy.savetxt('weights_' + '_epoch_' + str(epoch) + '_nin_' + str(n_visible) + '_nhdn_' + str(n_hidden) + '.csv', 
                      weights, delimiter = ',')
    ...
    sample_data = numpy.zeros([n_samples * n_chains, n_visible])
#    image_data = numpy.zeros(
#        ((n_visible + 5) * n_samples + 1, (5 + 1) * n_chains - 1),
#        dtype='uint8'
#    )
    for idx in xrange(n_samples):
        ...
        sample_data[idx * n_chains:(idx + 1) * n_chains, :] = vis_mf
#        image_data[(n_visible + 5) * idx:(5 + 1) * idx + 5, :] = tile_raster_images(
#            X=vis_mf,
#            img_shape=(1, n_visible),
