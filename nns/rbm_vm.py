...
def test_rbm(dataset = 'mnist.pkl.gz', learning_rate = 0.1, batch_size = 20, 
             output_folder='rbm_plots', n_hidden = 500, n_visible = 784, 
             learningRateBoostFactor = 1.5, deltaCostStopThreshold = 0.001, k = 15):
    """
    ...
    while deltaCost < -deltaCostStopThreshold or deltaCost < 0:
        ...
        biases = rbm.hbias.get_value(borrow = True)
        ...
        epochCost = numpy.mean(mean_cost)
        print 'Training epoch %d, cost is ' % epoch, epochCost
    ...
    outputFnRoot = 'k_%s_nin_%s_nhdn_%s_basz_%s_lnrt_%s' \
                % (str(k), str(n_visible), str(n_hidden), str(batch_size), str(origLearningRate))
