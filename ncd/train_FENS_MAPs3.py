...
# v2: added support for FENS parameters of quantisation steps and weights
# v3: added a transformation function to the FENS settings
...
# FENS settings
FENStransformationFunctions = ['square', 'cube', 'fourth', 'fifth', 'sixth', 
                               'seventh', 'eighth', 'ninth', 'tenth']
FENSwindowLengths = [10, 20, 30, 41, 50, 60, 70]
FENSdownsamplings = [1, 2, 4, 8, 10, 12]
FENSnormalisationThresholds = [0.0001, 0.0003, 0.001, 0.003, 0.01]
FENSquantisationStepBases = [0.01, 0.03, 0.05, 0.07, 0.09]
FENSquantisationStepPowers = [1.8, 1.9, 2.0, 2.1, 2.2] # these have been derived based on the top step not exceeding 1
FENSquantisationWeights1 = [0.5, 0.707, 1.0, 1.414, 2.0]
FENSquantisationWeights2 = [0.5, 0.707, 1.0, 1.414, 2.0]
FENSquantisationWeights3 = [0.5, 0.707, 1.0, 1.414, 2.0]
FENSquantisationWeights4 = [0.5, 0.707, 1.0, 1.414, 2.0]
FENScalcParameters = ['FENS Window Length', 'FENS Downsampling', 
                      'FENS Transformation Function',
                      'FENS Normalisation Threshold',
                      'FENS Quantisation Step Base' , 'FENS Quantisation Step Power',
                      'FENS Quantisation Weight 1', 'FENS Quantisation Weight 2',
                      'FENS Quantisation Weight 3', 'FENS Quantisation Weight 4',
                      'dA Num Hidden Units',  'dA Num Visible Units',
                      'dA Corruption Level'] # these categories trigger recalculation of FENS

# CRP settings
CRPmethod = 'fan'
CRPdimensions = [1, 2, 3, 4, 5]
CRPneighbourhoodSizes = [0.05, 0.1, 0.15, 0.2, 0.25]
...
numVisibles = [48]
numHiddens = [12]
corruptionLevels = [0, 10, 20, 30, 40]
timeStacking = 1
freqStd = False
NNnumFolders = 20
NNnumFilesPerFolder = 5

stopRunningAt = datetime(2015, 9, 23, 8)
...
settingsDict = {'Dimension': CRPdimensions,
                ...
                'FENS Transformation Function':FENStransformationFunctions,
                ...
                'FENS Quantisation Weight 2': FENSquantisationWeights2,
...
while currentDateTime < stopRunningAt:
    ...
    while nextSettings is not None and currentDateTime < stopRunningAt:
        ...
        if nextSettings is not None:
            print 'Trying %s' % opt.getCurrentSettingsParameter()
            for setting in nextSettings:
                ...
                if NNtype is not None:
                    weightMatrix, biases, featureOffset, featureScaling = get_NN_NCD_params(NNtype, 
                        ...
                        freqStd, NNnumFolders, NNnumFilesPerFolder, timeStacking)
                else:
                ...
                if opt.getCurrentSettingsParameter() in FENScalcParameters or FENSfeatureFileDict is None:
                    ...
                    FENSfeatureFileDict = FFP.generateFENSfeatureFileDict(
                                                ...
                                                FENStransformationFunction = setting['FENS Transformation Function'],
                                                FENSnormalisationThreshold = setting['FENS Normalisation Threshold'], 
                                                FENSquantisationSteps = FFP.getFENSQuantisationSteps(setting['FENS Quantisation Step Base'], 
                                                                                                     setting['FENS Quantisation Step Power']),
                                                FENSquantisationWeights = [setting['FENS Quantisation Weight 1'],
                                                                           setting['FENS Quantisation Weight 2'],
                                                                           setting['FENS Quantisation Weight 3'],
                                                                           setting['FENS Quantisation Weight 4']],
