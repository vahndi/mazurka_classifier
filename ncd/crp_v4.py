...
def normalise(X, doNormalisation):
    ...
    if doNormalisation == True:
        Xnorm = (X[:, 1:] - X[idx, 1:].mean()) / X[idx, 1:].std()
    else:
        Xnorm = X[:, 1:]
...
def getDistance(X, NX, Y, NY, dimension, timeDelay):
    ...
    for iRow in np.arange(NX):
        IX = np.tile(iRow + delays, [NY, 1]).transpose()
        arrXs = X[IX]
        iCols = np.tile(np.arange(NY), [delays.size, 1])
        rowDelays = np.tile(delays, [NY, 1]).transpose()
        IY = rowDelays + iCols
        arrYs = Y[IY]
        s[iRow, :, :] = np.sqrt(np.sum(np.power(arrXs - arrYs, 2), axis = 2).transpose())
...
def getDistanceNrmNorm(X, NX, Y, NY, dimension, timeDelay):
    ...
    for iRow in np.arange(NX):
        IX = np.tile(iRow + delays, [NY, 1]).transpose()
        arrXs = X[IX] / np.sqrt(np.power(X[IX], 2).sum(axis = 0))
        iCols = np.tile(np.arange(NY), [delays.size, 1])
        rowDelays = np.tile(delays, [NY, 1]).transpose()
        IY = rowDelays + iCols
        arrYs = Y[IY] / np.sqrt(np.power(Y[IY], 2).sum(axis = 0))
        s[iRow, :, :] = np.sqrt(np.sum(np.power(arrXs - arrYs, 2), axis = 2).transpose())
...
def crp_v4(x, y = None, 
    ...
    Changes:
        ...
        :v3:    Modified the normalise(), getDistance() and getDistanceNrmNorm() functions to handle
                multi-dimensional feature vectors.
        
        :v4:    Partially revectorised the getDistance() and getDistanceNrmNorm() functions to increase
                execution speed - iterations are done over entire rows instead of rows and columns
    '''
