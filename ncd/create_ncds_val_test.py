...
from paths import getTrainingSetFolders, getTrainingSetPerformances
from paths import getValidationSetFolders, getValidationSetPerformances
from paths import getTestSetFolders, getTestSetPerformances
...
def getFeatureFileDict(piecesPath, pieceFolder, featuresPath, featureName, runType):
    '''
...
def getFeatureFileDictAllFolders(rootPath, folderNames, featureName, runType):
    '''
    ...
    for pieceFolder in folderNames:
        ...
        ffDict = copy.deepcopy(getFeatureFileDict(rootPath, pieceFolder, featuresPath, featureName, runType))
...
def createCRPfile(crpProps):
    ...
    try:
        ...
        print 'Could not load file %s' % crpProps.featureFilePath
    
    try:
        # Crop number of features if necessary
        if crpProps.numFeatures is not None:
            x = x[:, :crpProps.numFeatures + 1]
    except:
        print 'Could not crop input features'
    ...
    try:
        ...
        if crpProps.weightMatrix is not None:
            ...
            if crpProps.timeStacking is not None and crpProps.timeStacking != 1:
                timeStacking = crpProps.timeStacking
        ...
        # Downsample
        x_ds = downSample(x_transformed, 
                          crpProps.downSampleFactor)
        
        # Calculate CRP
        y = crp_v4(x_ds, 
                   dimension = crpProps.dimension, 
                   timeDelay = crpProps.timeDelay, 
                   method = crpProps.method, 
        ...
        print 'Error creating CRP file %s' % outputFilePath
        return False
...
def createNCDfiles(existingNCDs, processPool,
    ...
    mazurkaIds = getFolderNames(mazurkasPath, contains = 'mazurka', orderAlphabetically = True)
    
    assert runType in ['training', 'validation', 'test']
    if runType == 'training':
        mazurkaIds = getTrainingSetFolders(mazurkaIds)
    elif runType == 'validation':
        mazurkaIds = getValidationSetFolders(mazurkaIds)
    elif runType == 'test':
        mazurkaIds = getTestSetFolders(mazurkaIds)
    ...
    featureFileDict = getFeatureFileDictAllFolders(mazurkasPath, mazurkaIds, featureName, runType)
